...
sortUniversity = graduates;
for(let i = 0; i < sortUniversity.length; i++ ) {
  universities.push(sortUniversity[i].university);
  universities.sort();
}
...
justContact = graduates;
for(let i = 0; i < justContact.length; i++) {
  contactInfo.push(justContact[i].first_name + " " + justContact[i].email) 
}
...
const scientificNames = zooAnimals.forEach(function(animal){
...
const allLower = zooAnimals.map(function(animal){
